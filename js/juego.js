 ...
 * VERSIÓN RESTAURADA COMPLETA (360+ LÍNEAS) - FIX IPHONE
 * ============================================================================
 */
...
class EscenaIntro extends Phaser.Scene {
    ...
    create() {
        ...
        // Posicionamiento basado en el centro de la cámara para evitar errores en iPhone
        const midX = this.cameras.main.centerX;
        const midY = this.cameras.main.centerY;
        
        // Fondo adaptativo
        this.add.image(midX, midY, 'fondo_menu')
        ...
        WebFont.load({
            google: {
                families: ['Cinzel Decorative', 'Eagle Lake']
            },
            active: () => {
                this.prepararSobre(midX, midY);
            }
        });
    }
        ...
        this.tweens.add({
            ...
            onComplete: () => {
                this.tweens.add({
                    ...
                    duration: 600,
                    onComplete: () => {
                        this.papel.setPosition(this.cameras.main.centerX, this.cameras.main.centerY);
    ...
    crearBotonEntrar() {
        ...
        const sello = this.add.circle(midX, midY, 50, 0x8b0000)
            .setInteractive({ useHandCursor: true })
            .setDepth(200);

        this.add.text(midX, midY, 'ENTRAR', { 
            fontSize: '20px', 
            fontFamily: 'Cinzel Decorative', 
            fill: '#ffd700' 
...
class EscenaSeleccion extends Phaser.Scene {
    ...
    create() {
        ...
        this.add.text(midX, 100, 'ELIGE TU PERSONAJE', { 
            fontSize: '40px', 
            fill: '#fff', 
            fontFamily: 'Cinzel Decorative' 
        }).setOrigin(0.5);

        const btnNovio = this.add.image(midX - 200, midY + 50, 'btn_novio')
            .setScale(1.8)
            .setInteractive({ useHandCursor: true });

        const btnNovia = this.add.image(midX + 200, midY + 50, 'btn_novia')
            .setScale(1.8)
            .setInteractive({ useHandCursor: true });

        btnNovio.on('pointerup', () => {
            personajeElegido = 'novio';
            this.scene.start('EscenaJuego');
        });

        btnNovia.on('pointerup', () => {
            personajeElegido = 'novia';
            this.scene.start('EscenaJuego');
        });
    }
}
...
class EscenaJuego extends Phaser.Scene {
    ...
    create() {
        this.input.addPointer(3); // Importante para iPhone multitouch
        ...
        this.bgLejano = this.add.tileSprite(0, 0, this.cameras.main.width, 600, 'lejano')
            .setOrigin(0).setScrollFactor(0);
        this.bgMedio = this.add.tileSprite(0, 0, this.cameras.main.width, 600, 'medio')
            .setOrigin(0).setScrollFactor(0);
        
        // Suelo y Físicas
        this.physics.world.setBounds(0, 0, anchoNivel, 600);
        this.sueloGroup = this.physics.add.staticGroup();
        for (let x = 0; x < anchoNivel; x += 32) {
            this.sueloGroup.create(x, 585, 'suelo').refreshBody();
        }
        ...
        // Personajes
        this.jugador = this.physics.add.sprite(100, 450, personajeElegido + '_frente')
            .setCollideWorldBounds(true).setDepth(100);
        
        const parejaImg = (personajeElegido === 'novio') ? 'novia_frente' : 'novio_frente';
        this.pareja = this.physics.add.sprite(5850, 450, parejaImg).setDepth(100);
        
        this.perro = this.physics.add.sprite(6000, 500, 'perro')
            .setScale(0.8).setAlpha(0).setDepth(150).setFlipX(true);
    ...
    crearControlesMovil() {
        ...
        const setupBtn = (x, y, txt, prop) => {
            let b = this.add.circle(x, y, 75, 0xffffff, 0.2)
                .setScrollFactor(0).setDepth(5000).setInteractive();
            this.add.text(x, y, txt, { fontSize: '60px' })
    ...
    configurarCajasYMonedas(coordsPlat) {
        this.textoCartel = this.add.text(0, 0, '', { 
            fontSize: '20px', fill: '#fff', fontFamily: 'Arial', fontWeight: 'bold',
            backgroundColor: '#8b0000', padding: { x: 15, y: 10 }, align: 'center', 
            wordWrap: { width: 300 }
        }).setOrigin(0.5, 1).setVisible(false).setDepth(2000).setStroke('#d4af37', 4);

        const texturaCaja = (personajeElegido === 'novio') ? 'caja_j' : 'caja_c';
        const mensajes = [
            "¡COMIENZA LA AVENTURA!", 
            "CUIDADO CON LAS POCIONES...", 
            "EL 4 DE JULIO DE 2026", 
            "EN ARANJUEZ NOS VEMOS", 
            "TRAED VUESTRAS VARITAS", 
            "¡VIVAN LOS NOVIOS!"
        ];
        ...
        for(let j=0; j<5; j++) {
            let sx = 600 + (j * 1100);
            let mSuelo = this.monedas.create(sx, 480, 'moneda_pixel').setScale(1.2);
            mSuelo.body.setAllowGravity(false);
        }
    ...
    vincularColisionCaja(c) {
        this.physics.add.collider(this.jugador, c, (obj1, obj2) => {
            if (obj1.body.touching.up && obj2.body.touching.down && c.disponible) { 
                c.disponible = false;
                this.sndCaja.play();
                ...
                this.time.delayedCall(2500, () => {
                    this.tweens.add({ targets: this.textoCartel, alpha: 0, duration: 500, onComplete: () => {
                        this.textoCartel.setVisible(false);
                        c.disponible = true;
                    }});
                });
            }
    ...
    crearObstaculos() {
        ...
        const posF = [1500, 2600, 3500, 4400, 5200];
        posF.forEach(dx => {
    ...
    finalizarJuego() {
        ...
        let corazonAnim = this.add.image(this.pareja.x - 20, this.pareja.y - 120, 'corazon').setDepth(1000);
        this.tweens.add({ targets: corazonAnim, scale: 8, alpha: 0, y: '-=250', duration: 2000 });
    ...
    escribirMensajePerro() {
        ...
        this.txtPerro = this.add.text(this.perro.x, this.perro.y - 120, "", { 
            ...
            align: 'center', fontWeight: 'bold', stroke: '#000', strokeThickness: 5
        ...
        this.time.addEvent({
            ...
            repeat: m.length - 1,
            callback: () => {
                this.txtPerro.text += m[charIndex];
                charIndex++;
    ...
    mostrarBotonesFinales() {
        ...
        let btnConf = this.add.image(midX - 200, midY, 'boton_confirmar')
            .setScrollFactor(0).setDepth(10000).setScale(0).setInteractive({useHandCursor:true});
        let btnMap = this.add.image(midX + 200, midY, 'boton_localizacion')
            .setScrollFactor(0).setDepth(10000).setScale(0).setInteractive({useHandCursor:true});
        ...
        btnMap.on('pointerup', () => { window.open(`http://maps.google.com`, '_blank'); });
    }
}
