...
class EscenaJuego extends Phaser.Scene {
    ...
    create() {
        ...
        this.textoPuntos = this.add.text(16, 16, 'MONEDAS: 0/10', { 
            fontSize: '24px', fill: '#fff', fontFamily: 'Cinzel Decorative', stroke: '#000', strokeThickness: 4 
    ...
    configurarCajasYMonedas(coordsPlat) {
        this.textoCartel = this.add.text(0, 0, '', { 
            ...
            backgroundColor: '#8b0000', padding: { x: 15, y: 10 }, align: 'center', wordWrap: { width: 300 }
        }).setOrigin(0.5, 1).setVisible(false).setDepth(2000).setStroke('#d4af37', 4);

        const texturaCaja = (personajeElegido === 'novio') ? 'caja_j' : 'caja_c';
        const mensajes = [
            "¡COMIENZA LA AVENTURA!", "CUIDADO CON LAS POCIONES...",
            "EL 4 DE JULIO DE 2026", "EN ARANJUEZ NOS VEMOS",
            "TRAED VUESTRAS VARITAS", "¡VIVAN LOS NOVIOS!"
        ];
    ...
    escribirMensajePerro() {
        ...
        this.time.addEvent({
            ...
            callback: () => {
                this.txtPerro.text += m[charIndex]; charIndex++;
                if (charIndex === m.length) { this.sndLadrido.stop(); this.mostrarBotonesFinales(); }
            }
        });
    }
